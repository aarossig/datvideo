...
/**
 * @file this CRC16 generator was taken from RFC1662 and modernized slightly.
 */
...
uint16_t GenerateIncrementalCrc16(uint16_t current_crc, uint8_t byte) {
  return (current_crc >> 8) ^ kCrcTable[(current_crc ^ byte) & 0xff];
}

}  // namespace

uint16_t GenerateCrc16(const uint8_t *buffer, size_t length) {
  uint16_t crc = kInitialCrc;

  while (length--) {
    crc = GenerateIncrementalCrc16(crc, *buffer++);
  }
